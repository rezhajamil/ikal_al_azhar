...
@section('body')
    <div class="w-full mx-4">
        <div class="flex flex-col">
            <div class="mt-4">
                <h4 class="text-xl font-bold text-gray-600 align-baseline">Edit Data Jurusan {{ $major->name }}</h4>

                <div class="px-6 py-4 mx-auto overflow-auto bg-white rounded-md shadow sm:mx-0 w-fit">
                    <form action="{{ route('admin.major.update', $major->id) }}" method="POST" class="">
                        @csrf
                        @method('put')
                        <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                                ...
                                <select name="faculty" id="faculty" class="w-full rounded-md" required>
                                    ...
                                    @foreach ($faculty as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('faculty', $major->faculty_id) == $item->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input class="w-full rounded-md form-input focus:border-indigo-600" type="text"
                                    name="name" value="{{ old('name', $major->name) }}">
                                @error('name')
                                    <span class="block mt-1 text-sm italic text-red-600">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="col-span-full">
                                ...
                                <input class="w-full rounded-md form-input focus:border-indigo-600" type="text"
                                    name="description" value="{{ old('description', $major->description) }}">
                                @error('description')
                                    <span class="block mt-1 text-sm italic text-red-600">{{ $message }}</span>
                                @enderror
                            </div>
